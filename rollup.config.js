...
// const google_cloud__local_auth_pkg = isProd
...
const BASE_CONFIG = {
  input: "src/main.ts",
  external: ["obsidian", "@google-cloud/local-auth", "googleapis"],
  onwarn: (warning, warn) => {
...
const DEV_PLUGIN_CONFIG = {
  ...BASE_CONFIG,
  output: {
    dir: "test-vault/.obsidian/plugins/calendar-sync-plugin",
    sourcemap: "inline",
    format: "cjs",
    exports: "default",
    name: "CalendarSync (Development)",
  },
  plugins: getRollupPlugins(
    undefined,
    copy({
      targets: [
        { src: "manifest.json", dest: "test-vault/.obsidian/plugins/calendar-sync-plugin" },
        { src: "styles.css", dest: "test-vault/.obsidian/plugins/calendar-sync-plugin" },
      ],
    })
  ),
};
